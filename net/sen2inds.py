...
trainFile = r"D:\研究生课件\大数据分析\FinalReoport\toutiao_train_set.txt"
...
def json2txt():
    ...
    # datas = list(filter(None, datas))
    datas = pd.read_csv(trainFile, sep=',')
    datas.columns = ['text', 'label']
    temp = datas
    datas = datas['text'].tolist()
    clss = temp['label'].tolist()
    random.shuffle(datas)
    for line in datas:
        # line = json.loads(line)
        # title = line['title']
        title = line
        idx = datas.index(line)
        cla_ind=int(clss[idx])
